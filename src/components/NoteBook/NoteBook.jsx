...
function NoteBook ({notes , onSelect , selectNotes}) {
    return (
        <div className="flex flex-col w-[41%] h-screen px-4 gap-8 flex-shrink-0">
            <div className="card-note-book w-full h-[16vh] bg-sky-100 shadow-lg shadow-sky-400 rounded-3xl p-6 flex justify-between items-center">
                ...
                <input className="w-[38%] h-[8vh] bg-white rounded-2xl p-4 text-green-900 font-medium border-2 border-green-900 shadow-2xl shadow-sky-500" type="text" placeholder="جستجو..."/>
            </div>
            <NoteList notes={notes} onSelect={onSelect} selectNotes={selectNotes} />
        </div>
    )
}
